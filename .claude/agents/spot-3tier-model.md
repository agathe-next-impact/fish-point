...
- ⏳ **Slices 3→6 — à faire** (détail ci-dessous). **Prochaine = slice 3** (reclassification data des accès : heuristique `osmTags` → `kind='ACCESS_ZONE'` + `parentId` par proximité PostGIS `ST_DWithin`, réversible, via script/`db execute` — action data sensible = orchestrateur).
