...
- ✅ **Slice 2 — filtre kind + défaut WATER_BODY (commit `9eb64e4`)** : `kind?: string[]` ajouté au vocab partagé `spot-filter-params.ts` (serialize/parse défensif via `KIND_SET`). Les **deux builders** appliquent le défaut : `buildSpotWhere` → `where.kind = { in: ['WATER_BODY'] }` (clé scalaire, hors AND) ; `buildSpotFilterSql` → fragment `s."kind"::text IN (...)` **poussé en dernier** (préserve l'ordre/index des autres fragments). Override explicite REMPLACE le défaut (pas de cumul). Schéma Zod liste : `SpotKindEnum` + `kind: z.array(...).default([])` + mapping `toSpotQueryFilters` + `raw.kind` dans `route.ts`. SEO `explore/[department]/(page|[commune]/page)` + `sitemap.ts` filtrés `kind: 'WATER_BODY'`. **12 fichiers** (8 source + 4 tests). Vérifié : tsc 0, eslint 0, vitest 271✓ (+14), runtime Neon (LIST def=53738/ACCESS_ZONE=0/both=53738, RAW idem → parité Prisma↔SQL + no-op). ⚠ Le dev server doit être redémarré pour que le `select`/where typé `kind` prenne (SQL brut non concerné).
- ❌ **Slice 3 — ABANDONNÉE (constat data 2026-06-20)** : la reclassification heuristique `osmTags → ACCESS_ZONE` est **inopérante** — `osmTags` est **NULL sur les 53 738 spots** en prod (jamais persisté à l'import, alors que `osm-ingestion.service.ts` le pose pourtant aujourd'hui → import historique plus ancien). `accessType` et `accessibility` aussi NULL à 100 %. Provenance : `overpass_osm` (47 287) + `hubeau_poisson` (6 377), **tous des plans d'eau / tronçons de rivière**, aucune infra d'accès. Reclasser sur le `name` = trop bruité (rejeté). Conclusion : impossible de peupler le niveau 2 depuis l'existant ; on ne peut que **capturer les accès à la création** → d'où le saut direct à la slice 5. (Re-peupler le niveau 2 nécessiterait une **ré-ingestion OSM** des features d'accès avec tags — option « Enrichir l'ingestion OSM », non retenue pour l'instant.)
- ✅ **Slice 5 — création kind-aware (commit `2fb8cb6`)** : `createSpotSchema.kind` (défaut WATER_BODY, rétro-compatible) ; `SpotForm` sélecteur de niveau + récap ; `POST /api/spots` persiste `kind` + résout `parentId` pour une ACCESS_ZONE ; `osm-ingestion.service.ts` pose `kind` (create + update) + `parentId` via `inferKindFromTags`. Helpers extraits : `src/lib/spot-hierarchy.ts` (`resolveParentWaterBodyId`, PostGIS `ST_DWithin` < 300 m, source unique route↔ingestion) et `src/lib/osm-kind.ts` (`inferKindFromTags` pur, testable sans la chaîne DB — sinon importer le service boote Prisma et casse les tests). **8 fichiers**. Vérifié : tsc 0, eslint 0 (2 warnings pré-existants : `watch()` RHF, `BBox` unused), vitest 282✓ (+8), runtime Neon (`resolveParent` retrouve le plan d'eau sur ses coords, NULL au large).
- ✅ **Slice 4 — affichage niveau 2 carte+fiche (commit `7cc6980`)** : fiche plan d'eau → bloc « Accès publics » (`SpotAccessZones`, liste les enfants) ; fiche accès → bandeau « Cet accès appartient à [plan d'eau] » (parent) ; `spots/[slug]/page.tsx` étend l'`include` (`parent` + `children` where kind=ACCESS_ZONE/APPROVED) et passe `accessZones`/`parentWaterBody` à `SpotDetail`. Carte : `AccessZoneLayer` (marqueurs violets `#6d28d9`, source `minzoom 13` → aucun fetch en vue large ; tuiles `?kind=ACCESS_ZONE`), clic réutilise le popup existant via `ACCESS_ZONE_LAYER_ID` dans `interactiveLayerIds` + `handleMapClick`. **6 fichiers** (2 new : `SpotAccessZones.tsx`, `AccessZoneLayer.tsx`). No-op visuel sans data. Vérifié : tsc 0, eslint 0, vitest 282, + runtime Neon via **fixture temporaire** (parent liste l'enfant, enfant résout le parent ; supprimé après, 0 reliquat). ⚠ Vérif visuelle (rendu carte/fiche dans le navigateur) reste à la charge de l'utilisateur.
- ✅ **Slice 6 — rattachement waypoint privé (commit `eb8b7b1`)** : migration M2 appliquée prod Neon (`private_spots.spotId` TEXT nullable + FK `spots(id)` ON DELETE SET NULL + index, fichier `prisma/migrations/20260621000000_add_private_spot_water_body/migration.sql`). Relation Prisma `PrivateSpotWaterBody` (unidirectionnelle ; back-relation `Spot.privateSpots` REQUISE par Prisma mais JAMAIS exposée publiquement). `private-spot.schema` : `spotId` optionnel/nullable. `POST /api/private-spots` : auto-résolution par proximité (`resolveParentWaterBodyId` <300m) si non fourni, choix client jamais écrasé ; PATCH inchangé (spread déjà compatible). `PrivateSpotForm` : note d'aide. Types src+shared. **8 fichiers**. Vérifié : colonne/FK SET NULL (`confdeltype='n'`)/index en base, 0 reliquat (rétro-compat) ; tsc 0, eslint 0, vitest 287 (+5) ; runtime end-to-end via fixture (create+auto-resolve+readback relation, supprimé).
- 🏁 **CHANTIER CLOS** : slices 0/1/2/4/5/6 faites ; slice 3 abandonnée (osmTags vide en prod, pas de data à reclasser). Modèle complet de bout en bout. **Niveau 2 (ACCESS_ZONE) invisible tant qu'aucune zone d'accès n'existe** — pour le peupler : créations utilisateurs (form kind-aware, slice 5) OU ré-ingestion OSM des features d'accès avec tags (`osm-ingestion` est déjà kind-aware, mais l'import historique n'a pas stocké les tags). Reste à la charge de l'utilisateur : vérif visuelle navigateur + `git push` (rien poussé).
